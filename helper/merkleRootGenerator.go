...
func GenerateMerkleRoot(transactions []model.Transaction) string {
	merkleTree := make([]string, len(transactions))

	for i, tx := range transactions {
		// Convert each Transaction to bytes
...
func transactionToBytes(transaction model.Transaction) []byte {
